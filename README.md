...
- a `[WARN]` summary of skipped folders (for example `AOI输出_*`, `research_bundle_*`, `研究输出_*`, and `输出结果` are treated as obvious non-scene result folders)
...
- `研究输出_YYYYMMDD_HHMMSS/scene_features_autogenerated.csv` when no manual `scene_features.csv` is supplied
- `研究输出_YYYYMMDD_HHMMSS/README_研究输出说明.txt` for the reading order and folder guide
