...
1. `tables/model_family_index.csv`
2. `tables/three_model_packet_summary.csv`
3. Then enter the three family folders in this order:
   - `01_main_effects/`
   - `02_two_way_interactions/`
   - `03_three_way_interaction/`
4. Within each family folder:
   - `tables/model_stability_summary.csv`
   - `png/evidence_stability_overview_<GroupVar>.png`
   - `png/evidence_model_fit_overview_<GroupVar>.png`
   - For each primary outcome:
     - `tables/model_fit_<outcome>.csv`
     - `tables/fixef_<outcome>.csv`
     - `tables/contrasts_<outcome>.csv` (interaction families first; main-effects family may omit or de-emphasize this)
     - `png/evidence_fixef_key_terms_<outcome>.png`
     - `png/evidence_contrasts_<outcome>.png` when contrasts are exported
   - `tables/ranef_<outcome>.csv` when variance decomposition is needed
   - `png/forest_fixef_<outcome>.png` for compact audit / appendix support
...
- use `tables/three_model_packet_summary.csv` to decide which model family and outcome are eligible for the main text
- within a family, use `tables/model_stability_summary.csv` to check stability before quoting effects
- use `tables/model_fit_<outcome>.csv` + `tables/fixef_<outcome>.csv` for the core statistical claim in that family
- use `tables/contrasts_<outcome>.csv` mainly for the two-way / three-way interaction families when you need simple-effect wording and reviewer response support
