...
def main() -> None:
    ap = argparse.ArgumentParser(description='One-command AOI batch runner for a single-scene input set')
    ap.add_argument('--csv_dir', required=True)
    ap.add_argument('--group_manifest', required=True)
    ap.add_argument('--scene_image', required=True)
    ap.add_argument('--aoi_json', required=True)
    ap.add_argument('--scene_id', default='scene1')
    ap.add_argument('--outdir', default='outputs_one_scene_bundle')
    ap.add_argument('--columns_map', default=None)
    ap.add_argument('--require_validity', action='store_true')
    ap.add_argument('--min_valid_ratio', type=float, default=0.6)
    ap.add_argument('--assume_clean', action='store_true')
    ap.add_argument('--time_segments', default='warn', choices=['warn', 'error', 'ignore'])
    args = ap.parse_args()
    ...
    cmd = [sys.executable, str(SCRIPTS / 'batch_aoi_metrics.py'), '--group_manifest', str(staged_manifest), '--scenes_root', str(stage_root), '--aoi_json_mode', 'image_stem', '--missing_aoi_json', 'error', '--unmatched_csv', 'error', '--outdir', str(batch_out), '--dwell_mode', 'fixation', '--point_source', 'fixation', '--screen_w', str(screen_w), '--screen_h', str(screen_h), '--image_match', 'error', '--min_valid_ratio', str(args.min_valid_ratio), '--time_segments', str(args.time_segments), '--report_time_segments', '--report_class_overlap', '--export_aoi_overlay', '--optimize_outputs']
    if args.columns_map:
    ...
    summary = {
        'inputs': {'csv_dir': str(csv_dir), 'group_manifest': str(group_manifest), 'scene_image': str(scene_image), 'aoi_json': str(aoi_json), 'scene_id': args.scene_id, 'csv_count': csv_count},
        'detected_scene_size': {'width': screen_w, 'height': screen_h},
        'outputs': {'batch_outputs': str(batch_out), 'class_csv': str(batch_out / 'batch_aoi_metrics_by_class.csv'), 'polygon_csv': str(batch_out / 'batch_aoi_metrics_by_polygon.csv'), 'optimized_outputs': str(batch_out / 'optimized_outputs'), 'aoi_overlays': str(batch_out / 'aoi_overlays')},
    }
    (outdir / 'run_summary.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
    print('Saved one-scene AOI bundle to:', outdir)
