    ...
    for _, r in m.iterrows():
        ...
        if 'Recording Time Stamp[ms]' in df.columns:
            ...
            if ts.size >= 2:
                ...
                if segments > 1:
                    msg = f"Detected timestamp gaps / multiple time segments for csv_path={r['csv_path']} (segments={segments}, neg_jumps={neg}, gap_jumps={gap}, gap_thr_ms={gap_thr}). File-level continuity is broken; TTFF will be recomputed segment-wise using Video Time when available, and QC fields will mark the segmentation."
