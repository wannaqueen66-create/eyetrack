#!/usr/bin/env python3
"""Explanatory AOI modeling (main effects + interactions + PNG outputs)."""
...
def main():
    ap = argparse.ArgumentParser(description="Explanatory AOI models with PNG outputs")
    ap.add_argument("--analysis_csv", required=True)
    ap.add_argument("--outdir", default="outputs_model_explanatory_pack")
    ...
    open(os.path.join(args.outdir, "RUNINFO.txt"), "w", encoding="utf-8").write("Explanatory AOI modeling\n" + f"analysis_csv={args.analysis_csv}\n")
